...
import com.developerobaida.tictactoe.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.image1.setOnClickListener(view -> {
            if (isBoxSelectable(0)) performAction((ImageView) view, 0);
        });
        binding.image2.setOnClickListener(view -> {
            if (isBoxSelectable(1)) performAction((ImageView) view, 1);
        });
        binding.image3.setOnClickListener(view -> {
            if (isBoxSelectable(2)) performAction((ImageView) view, 2);

        });
        binding.image4.setOnClickListener(view -> {
            if (isBoxSelectable(3)) performAction((ImageView) view, 3);
        });
        binding.image5.setOnClickListener(view -> {
            if (isBoxSelectable(4)) performAction((ImageView) view, 4);
        });
        binding.image6.setOnClickListener(view -> {
            if (isBoxSelectable(5)) performAction((ImageView) view, 5);
        });
        binding.image7.setOnClickListener(view -> {
            if (isBoxSelectable(6)) performAction((ImageView) view, 6);

        });
        binding.image8.setOnClickListener(view -> {
            if (isBoxSelectable(7)) performAction((ImageView) view, 7);

        });
        binding.image9.setOnClickListener(view -> {
            if (isBoxSelectable(8)) performAction((ImageView) view, 8);
        });
    }
    ...
    private boolean isBoxSelectable(int boxPosition) {
        boolean response = false;
        if (boxPositions[boxPosition] == 0) response = true;
